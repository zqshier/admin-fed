...
export function useFormEvents({
  ...
  async function setFieldsValue(values: Recordable): Promise<void> {
    ...
    //数组类型key
    const arrKeyArray = fields.filter((item) => item.indexOf('[') >= 0);
    ...
    arrKeyArray.forEach((arrKey: string) => {
      if (Reflect.has(values, arrKey)) return;
      const match = arrKey.match(/^\[(.+)\]$/);
      if (match && match[1]) {
        let keys = match[1].split(',');
        keys = keys.map((k) => k.trim());
        if (keys.some((k) => !Reflect.has(values, k))) return;
        const valueArr: any = [];
        keys.forEach((k) => {
          valueArr.push(values[k]);
        });
        formModel[arrKey] = valueArr;
        validKeys.push(arrKey);
      }
    });
